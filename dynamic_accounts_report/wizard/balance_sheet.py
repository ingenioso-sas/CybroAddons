...
class BalanceSheetView(models.TransientModel):
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        # Get account balances directly from SQL with aggregation
        ...
        account_results = {row['account_id']: row for row in cr.dictfetchall()}
        ...
        for account in accounts:
            ...
            res['id'] = account.id
            res['move_lines'] = [] # Empty for now to save memory, GL drill-down uses a separate action
            
            if account.id in account_results:
                res['debit'] = round(account_results[account.id].get('debit'), 2)
                res['credit'] = round(account_results[account.id].get('credit'), 2)
                res['balance'] = round(account_results[account.id].get('balance'), 2)
